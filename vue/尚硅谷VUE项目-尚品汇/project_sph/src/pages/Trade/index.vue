<template>
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix" v-for="item in adderss" :key="item.id">
...
export default {
  ...
  computed: {
    // 提交订单最终选中地址
    userDefaultAddress() {
      return this.adderss.find((item) => item.isDefault == 1) || {};
    },
  },
  methods: {
    // 提交订单
    ...
    // 获取用户地址信息
    async getFindUserAddressList() {
      let data = await trade_api.getFindUserAddressList();
      this.adderss = data;
    },
    // 获取商品清单
    async getTrade() {
      let data = await trade_api.getTrade();
      this.orderInfo = data;
    },
  },
  mounted() {
    this.getFindUserAddressList();
    this.getTrade();
  },
};
</script>
