    ...
    <div class="pay-container">
      ...
      <div class="checkout-steps">
        ...
        <div class="otherpay">
          ...
          <div class="step-cont">
            <span>
              <a href="weixin://wxpay/bizpayurl?pr=SMuoykTzz" target="_blank">微信支付</a>
            </span>
            <span>中国银联</span>
          </div>
        </div>
...
<script>
import trade_api from "@/api/trade";
...
export default {
  ...
  methods: {
    ...
    async open() {
      ...
      this.$alert(`<img src=${url} />`, "请使用微信支付", {
        ...
        beforeClose: (action, instance, done) => {
          console.log(action);
          // if (action == "confirm" && this.code != 200) return; // 先注释 方便开发 未支付也通过
      ...
      if (!this.time) {
        this.time = setInterval(async () => {
          let data = await trade_api.getQueryPayStatus(this.orderId);
          console.log(data);
          if (data == "支付成功") {
            clearInterval(this.time); // 清除定时器
            this.time = null;
            this.code = 200; // 保存返回的code
    ...
    async getCreateNative() {
      let data = await trade_api.getCreateNative(this.orderId);
      this.payInfo = data;
    },
  },
  mounted() {
